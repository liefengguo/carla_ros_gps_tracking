...
bool loadPath(string filename , vector<point> &waypoint) {
    ...
    for(int j=0;j<arr.size();j+=4)
    {
        ...
        temp_point.o_x = arr[j+3];
        temp_point.o_y = arr[j+4];
        temp_point.o_z = arr[j+5];
        temp_point.o_w = arr[j+6];
...
int main(int argc, char **argv) {
    ...
    while (ros::ok())
    {
        ...
        {
            cout<<waypoint[i].x<<"   "<<waypoint[i].y<<"   "<<waypoint[i].z<<endl;
        ...
        for(int i=0;i<waypoint.size();i++)
        {
            ...
            pose.pose.orientation.x = waypoint[i].o_x;
            pose.pose.orientation.y = waypoint[i].o_y;
            pose.pose.orientation.z = waypoint[i].o_z;
            pose.pose.orientation.w = waypoint[i].o_w;
